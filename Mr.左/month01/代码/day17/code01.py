...
"""
from custom_list_tools import ListHelper
...
max01 = ListHelper.get_max(list01,lambda e:e.hp)
print(max01.name)
...
for item in filter(lambda e:e.atk,list01):
    print("所有攻击力：",item.name,item.atk)

for item in filter(lambda e:10<e.hp>50,list01):
    print("血量在１０－５０：",item.name)

for item in sorted(list01,key=lambda e:e.atk):
    print("排序：",item.atk)

re=max(list01,key=lambda e:len(e.name))
print("名字字数：",re.name)
